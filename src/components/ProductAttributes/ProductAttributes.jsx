...
class BusinessesType extends Component {
  ...
  onChangeStatus = (confirm, status, id, idx) => {
    if (confirm) {
      ...
      this._productAttributeModel.changeStatus(id, postData)
      .then(() => {
  ...
  handleClose(confirm, id, rowIndex) {
    ...
    if (confirm) {
      this._productAttributeModel.delete(id)
      .then(() => {
  ...
  handleSubmitFilter = (search, is_active, is_deleted, from_date, to_date) => {
    ...
    query = Object.assign(query, {search, is_active, is_deleted, from_date, to_date});
    this.getData(query)
    .catch(() => {
      window._$g.dialogs.alert(
        window._$g._('Bạn vui lòng chọn dòng dữ liệu cần thao tác!')
  ...
  render() {
    const columns = [
      configIDRowTable("product_attribute_id", "/product-attributes/details/", this.state.query),
      {
        name: "attribute_name",
                ...
                <CheckAccess permission="PRO_PRODUCTATTRIBUTE_EDIT">
                  <FormControlLabel
                    ...
                    onChange={event => {
                      ...
                      this.handleChangeStatus(checked, this.state.data[tableMeta['rowIndex']].product_attribute_id, tableMeta['rowIndex'])
                    }}
                  />
                </CheckAccess>
              </div>
            ...
            return (
              <div className="text-center">
                <CheckAccess permission="PRO_PRODUCTATTRIBUTE_VIEW">
                  <Button color="warning" title="Chi tiết" className="mr-1" onClick={evt => this.handleActionItemClick('detail', this.state.data[tableMeta['rowIndex']].product_attribute_id, tableMeta['rowIndex'])}>
                    <i className="fa fa-info" />
                  </Button>
                </CheckAccess>
                <CheckAccess permission="PRO_PRODUCTATTRIBUTE_EDIT">
                  <Button color="primary" title="Chỉnh sửa" className="mr-1" onClick={evt => this.handleActionItemClick('edit', this.state.data[tableMeta['rowIndex']].product_attribute_id, tableMeta['rowIndex'])}>
                    <i className="fa fa-edit" />
                  </Button>
                </CheckAccess>
                <CheckAccess permission="PRO_PRODUCTATTRIBUTE_DEL">
                  <Button color="danger" title="Xóa" className="" onClick={evt => this.handleActionItemClick('delete', this.state.data[tableMeta['rowIndex']].product_attribute_id, tableMeta['rowIndex'])}>
